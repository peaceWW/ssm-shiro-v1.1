...
<script>
    layui.use(['table', 'element', 'jquery'], function () {
        ...
        table.on('tool(user)', function (obj) {
            var data = obj.data;
            if (obj.event === 'update') {
                window.location.href = '${pageContext.request.contextPath}/admin/permission/update/' + data.permissionId;
            }
            ...
            if (obj.event === 'delete') {
                ...
                layer.confirm('您确定要删除该权限吗？', {
                    ...
                    layer.closeAll();
                    $.post("${pageContext.request.contextPath}/admin/permission/deletePermissionById", {id: permissionId}, function (data) {
                        if (data == "success") {
                            //如果修改成功,则刷新页面
            ...
            if (obj.event === 'yes') {
                var permissionId = obj.data.permissionId;
                //已生成,设置为未生成
                $.post("${pageContext.request.contextPath}/admin/permission/updateFlag", {permissionId: permissionId, flag: 0}, function (data) {
                    var data = data;
                    if (data == "success") {
                        //如果修改成功,则刷新页面
                        //window.location.reload(); //刷新当前页面
                        window.location.href="${pageContext.request.contextPath}/admin/permission/index";
                    }
                })
            }
            if (obj.event === 'no') {
                var permissionId = obj.data.permissionId;
                //未生成,设置为已生成
                $.post("${pageContext.request.contextPath}/admin/permission/updateFlag", {permissionId: permissionId, flag: 1}, function (data) {
                    var data = data;
                    if (data == "success") {
                        //如果修改成功,则刷新页面
                        //window.location.reload(); //刷新当前页面
